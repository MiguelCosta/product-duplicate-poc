...
public class ProductService
{
    ...
    public ProductService(Data.GroupRepository groupRepository)
    {
        _groupRepository = groupRepository;
    }

    public async Task<List<string>> UpdateProductsRandomAsync(
        int totalProductsToUpdate,
        CancellationToken cancellationToken)
    {
        var minId = 1_000_000;
        var maxId = 1_000_000 + 5_000_000;

        var f = new Random();
        var updates = new List<(string ItemId, string NewDescription)>();

        for (var i = 0; i < totalProductsToUpdate; i++)
        {
            var itemId = f.Next(minId, maxId).ToString();
            updates.Add((itemId, "Updated description " + DateTime.UtcNow.Ticks));
        }

        await _groupRepository.UpdateAsync(updates, cancellationToken);

        return updates.Select(x => x.ItemId).ToList();
    }
